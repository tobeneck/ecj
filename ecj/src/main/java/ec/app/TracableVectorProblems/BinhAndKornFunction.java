...
public class BinhAndKornFunction extends Problem implements SimpleProblemForm
{
                         ...
                         final int threadnum)
    {
        ...
        if( len != 2 )
            state.output.fatal( "The BinhAndKornFunction only works with a genome length of 2, not of: "+ len );

        double x = genome[0].getValue();
        double y = genome[1].getValue();
        double obj1 = (4*(x*x)) + ((4*(y*y))); // 4*x² + 4*y²
        double obj2 = ((x-5)*(x-5)) + ((y-5)*(y-5)); // (x-5)² + (y-5)²
