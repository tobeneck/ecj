...
public class Transformations {
     ...
     *
     */
    public float bPoly(float y, float alpha, EvolutionState state) {
        if (!(alpha > 0)) {

            Class<String> cls = String.class;
            String name = cls.getName();

            state.output.fatal("wfg.Transformations.bPoly: Param alpha " +
                    "must be > 0 \n" + "Exception in " + name + ".bPoly()");
        }
    ...
    public float rSum(float[] y, float[] w) {
        ...
        //TODO: remove this, dunno if it is actually acurate
        //if(tmp1 == 0.0 && tmp2 == 0.0)
        //    return (float)0.0;

        //TODO: it should not happen that y and w are empty, should it?
