...
public class UF2 extends MultiObjectiveTraceableDoubleVectorProblem {
    ...
    public void initialSetup(final EvolutionState state, final Parameter base) {
        ...
        this.problemName = "UF2"; //the name of the problem
    }
    ...
    protected double[] getFitness(TraceableDouble[] genome, int numberObjectives, EvolutionState state){
        ...
        for (int j = 2 ; j <= genomeLength; j++) {
            if(j % 2 == 0) {
                yj = x[j-1] -
                        (0.3 * x[0] * x[0] * Math.cos(24 * Math.PI * x[0] + 4 * j * Math.PI / genomeLength) + 0.6 * x[0])*
                                Math.sin(6.0 * Math.PI* x[0] + j * Math.PI / genomeLength);
                sum2 += yj*yj;
                count2++;
            } else {
                ...
                yj = x[j-1] -
                        (0.3 * x[0] * x[0] * Math.cos(24 * Math.PI * x[0] + 4 * j * Math.PI / genomeLength) + 0.6 * x[0])*
                                Math.cos(6.0 * Math.PI* x[0] + j * Math.PI / genomeLength);
