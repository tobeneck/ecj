 ...
 * also mix ESSelection operators with other operators (like Tournament Selection).  But you tought
...
public class CorrectMuCommaLambdaBreeder extends Breeder
{
...
//        }
        ...
        else
        {
            ...
            for(int y=0;y<numThreads;y++)
            {
                CorrectMuLambdaBreederThread r = new CorrectMuLambdaBreederThread();
                r.threadnum = y;
                r.newpop = newpop;
                r.numinds = numinds[y];
                r.from = from[y];
    ...
    {
        //filter here to be able to call it from Mu+Lambda
        //TODO: filter for the 20 best. Or worst. Or something like that
        //TODO: check how Mu and Lambda are build in the initial breeder. Basically, mu=parents and lambda=offspring.
        //TODO: from this we can implement mu+lambda and mu,lanbda, as well as elites and stuff like that. Nice right?
        ArrayList<Individual> newpopindividuals = newpop.subpops.get(0).individuals;
        System.out.println(newpopindividuals.size());
        while(newpopindividuals.size() > 20){
            newpopindividuals.remove(0);
        }
        System.out.println(newpopindividuals.size());
...
class CorrectMuLambdaBreederThread implements Runnable
{
    Population newpop;
    public int[] numinds;
    public int[] from;
    public CorrectMuCommaLambdaBreeder me;
